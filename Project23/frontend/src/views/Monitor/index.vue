  ...
  <div class="real-time-monitor">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-select v-model="selectedDevice" placeholder="选择设备" style="width: 200px; margin-right: 16px">
          <el-option
      ...
      <el-col :span="6">
        <div class="metric-card temperature">
          ...
          <div class="metric-body">
            <span class="value" :class="{ warning: currentMetrics.temperature > 80 }">
          ...
          <div class="metric-footer">
            ...
            <el-progress
              :percentage="Math.min(100, (currentMetrics.temperature / 100) * 100)"
              :color="currentMetrics.temperature > 80 ? '#F56C6C' : '#67C23A'"
              :stroke-width="6"
              :show-text="false"
            />
          </div>
...
function initData() {
  ...
  for (let i = 59; i >= 0; i--) {
    ...
    monitorStore.updateRealtimeData('temperature', 65 + Math.random() * 15, timestamp)
    monitorStore.updateRealtimeData('vibration', 3 + Math.random() * 3, timestamp)
    monitorStore.updateRealtimeData('pressure', 0.4 + Math.random() * 0.3, timestamp)
    monitorStore.updateRealtimeData('current', 18 + Math.random() * 10, timestamp)
  }
...
function autoGenerateData() {
  mockTimer = setInterval(() => {
    ...
    currentMetrics.temperature = clamp(currentMetrics.temperature + (Math.random() - 0.5) * 2, 40, 95)
    currentMetrics.vibration = clamp(currentMetrics.vibration + (Math.random() - 0.5) * 0.5, 1, 8)
    currentMetrics.pressure = clamp(currentMetrics.pressure + (Math.random() - 0.5) * 0.05, 0.2, 1)
    currentMetrics.current = clamp(currentMetrics.current + (Math.random() - 0.5) * 1.5, 8, 38)

    monitorStore.updateRealtimeData('temperature', currentMetrics.temperature, timestamp)
    monitorStore.updateRealtimeData('vibration', currentMetrics.vibration, timestamp)
    monitorStore.updateRealtimeData('pressure', currentMetrics.pressure, timestamp)
    monitorStore.updateRealtimeData('current', currentMetrics.current, timestamp)
  }, 1000)
}
...
function updateCurrentMetrics() {
  ...
  if (tempData.length) currentMetrics.temperature = tempData[tempData.length - 1].value
...
async function toggleConnection() {
  if (wsConnected.value) {
    ...
    try {
      ...
      ElMessage.warning('WebSocket连接失败，使用模拟数据')
    }
  }
}
